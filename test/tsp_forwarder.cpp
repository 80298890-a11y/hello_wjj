...
std::string get_topic_log_path(const std::string& topic_name) {
    ...
    return "tsp_forwarder/" + path_name;
}
...
std::string addMQTTTimestamp(const std::string& original_json) {
    try {
        ...
        // 如果原消息缺少timestamp字段，添加timestamp
        if (!root.isMember("timestamp") || root["timestamp"].isNull()) {
            root["timestamp"] = current_timestamp;
            LOG_INFO("自动添加timestamp字段: " + std::to_string(current_timestamp));
        }
        
        // 添加MQTT发送时间戳
        root["mqtt_send_timestamp"] = current_timestamp;
        
        Json::FastWriter writer;
        return writer.write(root);
        
    } catch (const std::exception& e) {
        LOG_ERROR("JSON处理异常: " + std::string(e.what()));
        return original_json;
    }
}
...
void on_message(struct mosquitto* mosq, void* userdata, const struct mosquitto_message* message) {
    ...
    LOG_BY_TOPIC(get_topic_log_path(topic), "接收到消息 [" + topic + "]: " + payload);
