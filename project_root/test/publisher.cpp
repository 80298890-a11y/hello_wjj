...
void send_test_messages() {
    ...
    long override_response = (count % 3 == 0) ? 1 : -1;
    bool control_source = (count % 2 == 1);
    if (g_publisher->publishHandshakeResponse("/handshake/response", noa_response, override_response, control_source, timestamp)) {
        std::string msg = "HandshakeResponse发送成功: noa_response=" + std::to_string(noa_response) + 
                         ...
                         ", control_source=" + std::to_string(control_source) + 
