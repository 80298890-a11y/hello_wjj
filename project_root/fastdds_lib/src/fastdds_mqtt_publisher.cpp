...
{
    // 1. 发布到FastDDS (调用父类方法)
    if (!FastDDSPublisher::publishRemoteControl(topic, steering_angle_enable, steering_angle, target_acceleration_enable, target_acceleration, indicator_left_enable, indicator_left, indicator_right_enable, indicator_right, gear_position_enable, gear_position, ebrake_status_enable, ebrake_status, lane_change, timestamp)) {
        return false;
    }
...
std::string FastDDSMQTTPublisher::createHandshakeRequestMQTTPayload(
    long noa_active_request, long remote_override_status, long remote_override_ready, uint64_t timestamp)
{
    return "{"
           "\"noa_active_request\":" + std::to_string(noa_active_request) + ","
           "\"remote_override_status\":" + std::to_string(remote_override_status) + ","
           "\"remote_override_ready\":" + std::to_string(remote_override_ready) + ","
           "\"timestamp\":" + std::to_string(timestamp) +
           "}";
}

std::string FastDDSMQTTPublisher::createHandshakeResponseMQTTPayload(
    long noa_active_response, long remote_override_response, bool current_control_source, uint64_t timestamp)
{
    return "{"
           "\"noa_active_response\":" + std::to_string(noa_active_response) + ","
...
std::string FastDDSMQTTPublisher::createVehicleStatusMQTTPayload(
    uint32_t vehicle_id, float speed, int control_mode, double longitude, double latitude,
    float altitude, double heading, int gear_position, int power_mode, uint64_t timestamp)
{
    // 按照IDL定义的完整17个字段
    return "{"
