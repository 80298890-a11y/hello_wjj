...
bool MQTTToFastDDSConverter::convertHandshakeRequest(const Json::Value& json) {
    ...
    std::cout << "[PROC] [HandshakeRequest] 开始发布到FastDDS (完整4字段): noa_active_request=" << noa_active_request 
              << ", remote_override_status=" << remote_override_status 
              << ", remote_override_ready=" << remote_override_ready 
    ...
    if (result) {
        std::cout << "[SUCCESS] [HandshakeRequest] FastDDS发布成功 (完整4字段): topic='/handshake/request'" << std::endl;
    } else {
...
bool MQTTToFastDDSConverter::convertRemoteControl(const Json::Value& json) {
    ...
    std::cout << "[PROC] [RemoteControl] 开始发布到FastDDS (完整14字段): "
              << "steering_enable=" << (steering_angle_enable ? "true" : "false")
              << ", steering_angle=" << steering_angle 
              << ", accel_enable=" << (target_acceleration_enable ? "true" : "false")
              << ", acceleration=" << target_acceleration
              << ", left_indicator_enable=" << (indicator_left_enable ? "true" : "false")
              << ", left_indicator=" << (indicator_left ? "true" : "false")
              << ", right_indicator_enable=" << (indicator_right_enable ? "true" : "false")
              << ", right_indicator=" << (indicator_right ? "true" : "false")
              << ", gear_enable=" << (gear_position_enable ? "true" : "false")
              << ", gear=" << gear_position
              << ", ebrake_enable=" << (ebrake_status_enable ? "true" : "false")
              << ", ebrake=" << ebrake_status
    ...
    if (result) {
        std::cout << "[SUCCESS] [RemoteControl] FastDDS发布成功 (完整14字段): topic='/vehicle/control_cmd'" << std::endl;
    } else {
