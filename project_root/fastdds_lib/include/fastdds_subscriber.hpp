...
public:
    // 回调函数类型定义
    using HandshakeRequestCallback = std::function<void(long noa_active_request, long remote_override_status, long remote_override_ready, uint64_t timestamp)>;
    using HandshakeResponseCallback = std::function<void(long noa_active_response, long remote_override_response, bool current_control_source, uint64_t timestamp)>;
    using VehicleStatusCallback = std::function<void(unsigned long vehicle_id, long control_mode, double position_longitude, double position_latitude, float position_altitude, float speed, float yawrate, long gear_position, float acceleration, double heading, float steering_angle, float wheel_angle, long ebrake_status, bool indicator_left, bool indicator_right, uint64_t timestamp, int power_mode)>;
    ...
    using DriveStateCallback = std::function<void(long driving_state, uint64_t timestamp)>;
    using FaultReportCallback = std::function<void(const std::vector<int32_t>& fault_types, uint64_t timestamp)>;
     ...
     * @brief 设置故障报告消息回调
     */
    void setFaultReportCallback(FaultReportCallback callback);
     ...
     * @return 成功返回true，失败返回false
     */
    virtual bool subscribeDriveState(const std::string& topic = "/vehicle/drive_state");
     ...
     * @return 成功返回true，失败返回false
     */
    virtual bool subscribeFaultReport(const std::string& topic = "/vehicle/fault_report");
...
protected:
    ...
    class DriveStateListener;
    class FaultReportListener;
...
private:
    ...
    FaultReportCallback fault_report_callback_;
